...
static const uint8_t A144[D_VAL * D_VAL] = {
    0x1, 0x2, 0x8, 0x5, 0x8, 0x2, 0x2, 0x5, 0x1, 0x2, 0x6, 0xC,
    0xC, 0x9, 0xF, 0x8, 0x8, 0xD, 0xD, 0x5, 0xB, 0x3, 0xA, 0x1,
    0x1, 0xF, 0xD, 0xE, 0xB, 0x8, 0x8, 0x2, 0x3, 0x3, 0x2, 0x8};
...
static const uint8_t RC[12] = {1, 3, 7, 14, 13, 11, 6, 12, 9, 2, 5, 10};
...
static uint8_t mul(uint8_t a, uint8_t b) {
    ...
    if (ret > 0x0F) {
        ...
        if (a < b) {
            SWAP(a, b);
        }  // Want a to be larger to reduce computation

        if (b == 0) {
            return b;
        }  // Multiply a by 0 is 0
        if (b == 1) {
            return a;
        }  // Multiply a by 1 is a
        ...
        while (b > 1) {
            ...
            if (MMUL[loc] < 0x10) {
                ...
                if (MMUL[32 + loc] > 0xF) {
                    ret = ret << 1;
                    // XOR with constant if shifted results is larger than 4
                    // bits
                    if (ret & 0x10) {
                        ret ^= G_CON;
                    }
